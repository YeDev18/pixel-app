...
const Card = ({ arrayItemsCard }: ArrayItemsCardProps) => {
	...
	return (
		<>
			<div
				className={`max-lg:card  relative  min-h-[30rem] min-w-48 place-items-center overflow-hidden rounded-[2.5rem] border border-blue-900   bg-cover bg-center text-light-100 shadow-sm ${arrayItemsCard.id === 1 && "row-span-8"} ${arrayItemsCard.id === 3 && "row-span-8"} ${arrayItemsCard.id === 4 && "row-span-8"} `}
				style={{
